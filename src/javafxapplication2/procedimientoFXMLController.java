 ...
 * @author racli
 */
public class procedimientoFXMLController implements Initializable {
    @FXML
    ...
    public ObservableList<Procedimiento> data;
    public ArrayList<Procedimiento> lista = new ArrayList<>(4);
    @FXML
 ...
 @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            bd.conectarDoctor(controlador.getContra());
             ResultSet p = bd.consultar("SELECT distinct causaproc FROM procedimiento order by causaproc");
                while (p.next()) {
                   
                    Procedimiento nuevo = new Procedimiento(p.getString(1));
                    lista.add(nuevo);
                }
            ...
            Logger.getLogger(procedimientoFXMLController.class.getName()).log(Level.SEVERE, null, e);
        } catch (ClassNotFoundException eTx) {
            Logger.getLogger(procedimientoFXMLController.class.getName()).log(Level.SEVERE, null, eTx);
        }
   ...
   public void nuevotipo() throws SQLException{
     if(codigo.getText().trim().isEmpty()){
        ...
        Procedimiento nuevo=new Procedimiento(codigo.getText());
         if(data.contains(nuevo)){
               cartelaviso.setText("   Resultado ya existente");
              cartelaviso.setVisible(true);
         }else{
                cartelaviso.setVisible(false);  
       
              ResultSet p=  bd.consultar("select insertar_procedimiento('"+codigo.getText()+"')");
//        if(p==null){
//                cartelaviso.setText(" Enfermedad ya existente");
//              cartelaviso.setVisible(true);
//        }else{
            if(p.next()){
                data.add(new Procedimiento(codigo.getText()));
            cerrarCambio();
        }//}
...
public void eliminar(){
      Procedimiento nuevo=tabla.getSelectionModel().getSelectedItem();
   try {
      ResultSet p= bd.consultar("SELECT eliminar_procedimiento('" +nuevo.getCausa()+"')");
 if(p!=null){
               data.remove(nuevo);
           }
